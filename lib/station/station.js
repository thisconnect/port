var pd = require('./modules/pd').Wrapper,
	planet = require('./modules/planet').Client;;
...
var receiver = pd.listen(8003),
	puredata = pd.start(getPdPath(), ['-path', '../blib/', '-noprefs', './base.pd']);
...
// logs data from [print] in '-stderr' mode
...
receiver.on('connection', function(socket){
	...
	sender.on('connect', function(){
		var client = planet.connect('//:8006'),
			state = {};
